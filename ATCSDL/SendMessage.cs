...
namespace ATCSDL
{
    public partial class SendMessage : Form
    {
        ...
        private List<Mes> GetMessage()
        {
            List<Mes> datalist = new List<Mes>();

            string queryString = "SELECT * FROM Message WHERE LoginSupplier = @LoginSupplier AND LoginCustomer = @LoginCustomer";
        ...
        private void sendMesTxt_TextChanged(object sender, EventArgs e)
        {
            if (sendMesTxt.Text.Length > 150)
            {
                sendMesTxt.Text = sendMesTxt.Text.Substring(0, 150);
                sendMesTxt.SelectionStart = 150;
                sendMesTxt.SelectionLength = 0;
            }
        }
    }
}
