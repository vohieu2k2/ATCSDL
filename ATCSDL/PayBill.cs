...
namespace ATCSDL
{
    public partial class PayBill : Form
    {
        ...
        private void DirectBuy_Load(object sender, EventArgs e)
        {
            ...
            LoadTransportIntoComboBox(transportCb);
        ...
        private void addCartBtn_Click(object sender, EventArgs e)
        {
            foreach (var loginSupplier in loginSuppliers)
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = @"INSERT INTO [Order] (NameOrder, TelephoneOrder, PriceOrder, DateOrder, AddressOrder, LoginCustomer, IDPayment, IDTransportation, StatusOrder, LoginSupplier)
                         OUTPUT INSERTED.IDOrder
                         VALUES (@NameOrder, @TelephoneOrder, @PriceOrder, @DateOrder, @AddressOrder, @LoginCustomer, @IDPayment, @IDTransportation, @StatusOrder, @LoginSupplier)";

                    using (SqlCommand cmd = new SqlCommand(query, connection))
                    {
                        connection.Open();

                        cmd.Parameters.AddWithValue("@NameOrder", nameCustomerTxt.Text);
                        cmd.Parameters.AddWithValue("@TelephoneOrder", phoneTxt.Text);
                        ...
                        {
                            cmd.Parameters.AddWithValue("@IDTransportation", transportCb.SelectedValue);
                        }
                        else
                        {
                            cmd.Parameters.AddWithValue("@IDTransportation", (int)transportCb.SelectedValue + 3);
        ...
        {
            int selectedValue = (int)transportCb.SelectedValue;
            if (selectedValue == 1)
            {
                if (addressTxt.Text.Contains("Hồ Chí Minh") || addressTxt.Text.Contains("Hà Nội"))
                {
            ...
            else if (selectedValue == 2)
            {
                if (addressTxt.Text.Contains("Hồ Chí Minh") || addressTxt.Text.Contains("Hà Nội"))
                {
                    transportMoney = 20000;
                }
                else
                {
                    transportMoney = 30000;
                }
            }
        ...
        private void LoadTransportIntoComboBox(System.Windows.Forms.ComboBox comboBox)
        {
            List<Transport> deliveryMethods = new List<Transport>()
                {
                    new Transport("Giao hàng tiêu chuẩn", 1),
                    new Transport("Giao hàng nhanh", 2),
                    new Transport("Giao hàng hỏa tốc", 3)
                };

            comboBox.DataSource = deliveryMethods;
        ...
        private void nameCustomerTxt_TextChanged(object sender, EventArgs e)
        {
            if (nameCustomerTxt.Text.Length > 50)
            {
                nameCustomerTxt.Text = nameCustomerTxt.Text.Substring(0, 50);
                nameCustomerTxt.SelectionStart = 50;
                nameCustomerTxt.SelectionLength = 0;
            }
        }
        ...
        private void transportCb_SelectedValueChanged(object sender, EventArgs e)
        {
            int selectedValue;
            if (int.TryParse(transportCb.SelectedValue.ToString(), out selectedValue))
            {
                CalculateMoney();
            }
        }
